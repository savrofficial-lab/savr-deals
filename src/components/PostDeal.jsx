...
export default function PostDeal({ onPosted }) {
  ...
  async function handleSubmit(e) {
    ...
    if (form.category && !CATEGORIES.includes(form.category)) {
      alert("⚠️ Please select a valid category from the dropdown list.");
      return;
    }
            ...
            <div className="relative">
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="Start typing or click to see all categories..."
                  className={`w-full border-2 rounded-xl px-4 py-3.5 pr-10 text-gray-800 placeholder-gray-400 transition-all duration-300 ${
                ...
                <div className="absolute right-4 top-1/2 transform -translate-y-1/2 pointer-events-none">
                  <svg 
                    className={`w-5 h-5 text-gray-400 transition-transform duration-300 ${showCategoryDropdown ? 'rotate-180' : ''}`} 
                    fill="none" 
                    stroke="currentColor" 
                    viewBox="0 0 24 24"
                  >
                ...
                {showCategoryDropdown && (
                  <div className="absolute z-10 w-full mt-2 bg-white border-2 border-blue-200 rounded-xl shadow-2xl max-h-60 overflow-y-auto">
                    {filteredCategories.length > 0 ? (
                      filteredCategories.map((category, index) => (
                        <div
                          key={index}
                          onMouseDown={() => selectCategory(category)}
                          className="px-4 py-3 hover:bg-blue-50 cursor-pointer transition-colors duration-150 border-b border-gray-100 last:border-b-0 flex items-center justify-between group"
                        >
