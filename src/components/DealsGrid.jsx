...
export default function DealsGrid({
  ...
  // keep an internal selectedCategory but sync from external prop if provided
  const [selectedCategory, setSelectedCategory] = useState(externalSelectedCategory ?? "All");
  const [loading, setLoading] = useState(true);
  const [errorMsg, setErrorMsg] = useState("");

  // sync external selectedCategory -> local
  useEffect(() => {
    setSelectedCategory(externalSelectedCategory ?? "All");
  ...
  return (
    <div>
      {/* Category buttons (hide when parent asked to hide them) */}
          ...
          return (
            ...
            >
              <div className="relative">
                <img src={imageSrc} alt={deal.title || "Deal image"} loading="lazy" className="w-full h-36 object-contain mb-3" />
                {discountBadge && <div className="absolute left-3 top-3 bg-yellow-800 text-white text-xs font-semibold px-2 py-1 rounded">{discountBadge}</div>}
              </div>

              <h3 className="text-sm sm:text-base font-semibold text-gray-800 mb-1 line-clamp-2">{deal.title}</h3>

              {deal.description && <p className="text-xs text-gray-600 mb-2 line-clamp-3">{deal.description}</p>}
              ...
              <div className="mt-auto flex items-center justify-between gap-3">
                ...
                <Link to={`/deal/${deal.id}`} className="px-3 py-2 bg-yellow-800 text-white rounded">
                  View Deal
                </Link>
              </div>
            </div>
