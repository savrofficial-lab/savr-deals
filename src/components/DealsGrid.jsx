...
export default function DealsGrid({ search }) {
  ...
  async function fetchDeals() {
    setLoading(true);
    let query = supabase.from("deals").select("*").eq("published", true).order("created_at", { ascending: false });
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {deals.map((deal) => (
        <div key={deal.id} className="bg-white rounded-xl shadow hover:shadow-lg transition overflow-hidden">
          <img
          ...
          <div className="p-4 flex flex-col gap-2">
            ...
            <div className="flex justify-between items-center">
              <span className="text-yellow-800 font-bold">₹{deal.price}</span>
              <a
                href={deal.link}
                target="_blank"
